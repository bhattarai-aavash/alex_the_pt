...
import { Dumbbell, Salad, RefreshCw, Target, Plus, Activity } from "lucide-react"
...
const NAV_ITEMS = [
  { key: "workouts" as const, icon: Dumbbell, label: "Workouts", color: "text-amber-400", bg: "bg-amber-400/10" },
  { key: "nutrition" as const, icon: Salad, label: "Nutrition", color: "text-green-400", bg: "bg-green-400/10" },
  { key: "recovery" as const, icon: RefreshCw, label: "Recovery", color: "text-blue-400", bg: "bg-blue-400/10" },
  { key: "form" as const, icon: Target, label: "Form", color: "text-purple-400", bg: "bg-purple-400/10" },
]

export function Sidebar({ onNewChat, messageCount, activeTopic, onSelectTopic }: SidebarProps) {
  return (
    <aside className="hidden md:flex flex-col w-64 border-r border-border bg-card/50 backdrop-blur-sm">
      ...
      <div className="px-3 py-3">
        ...
        <nav className="space-y-0.5">
          {NAV_ITEMS.map(({ key, icon: Icon, label, color, bg }) => (
            <button
              key={label}
              type="button"
              onClick={() => onSelectTopic(key)}
              className={cn(
                "w-full flex items-center gap-3 px-2 py-2 rounded-lg text-sm transition-all duration-150 group",
                activeTopic === key
      ...
      <div className="px-5 py-4 border-t border-border">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-muted-foreground font-mono">Powered by LangGraph</span>
        </div>
      </div>
    </aside>
  )
