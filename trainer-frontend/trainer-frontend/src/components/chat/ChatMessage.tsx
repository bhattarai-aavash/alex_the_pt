...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  return (
    <div className={cn("message-enter flex gap-3 px-4 py-3", isUser ? "flex-row-reverse" : "flex-row")}>
      ...
      >
        ...
        {message.isTyping ? (
          ...
          <div className="prose prose-sm max-w-none dark:prose-invert prose-p:my-2 prose-ul:my-2 prose-ol:my-2 prose-headings:mt-3 prose-headings:mb-2 prose-strong:text-foreground dark:prose-strong:text-foreground">
            <ReactMarkdown remarkPlugins={[remarkGfm]}>{message.content}</ReactMarkdown>
          </div>
        )}

        {!message.isTyping && (
          <p className={cn("mt-1.5 text-[10px] font-mono opacity-40", isUser ? "text-right" : "text-left")}>
